...
_SAMPLE_PROTOCOL = "prefix_positive_targets_v4"
_ID_DTYPE = np.uint16
...
def process_features_for_ranking(df_movies, df_ratings, df_users):
    ...
    df_movies["genres"] = df_movies["genres_raw"].str.split("|").str[0]
    ...
    movie_vocab = {}
    for feat_name in ["movie_id", "genres", "isAdult", "startYear"]:
        label_encoder = LabelEncoder()
    ...
    movie_vocab["genres_multi"] = genre_token_encoder.classes_
    ...
    df_merged = df_merged.merge(
        df_movies[["movie_id", "movie_id_encoded", "genres_encoded", "isAdult_encoded", "startYear_encoded", "startYear_raw", "genre_tokens_encoded"]],
        on="movie_id",
        how="left",
    )
...
def _allocate_split_arrays(sample_count: int, max_seq_len: int) -> dict[str, np.ndarray]:
    return {
        ...
        "context_genres": np.zeros((sample_count, max_seq_len), dtype=_ID_DTYPE),
...
def _write_sample_row(
    ...
    store["context_genres"][row_idx].fill(0)
    ...
    _fill_padded_row(store["context_genres"][row_idx], context_genres)
...
def _build_item_feature_arrays(df_merged: pd.DataFrame, total_item_count: int) -> dict[str, np.ndarray]:
    item_features = {
        "genres": np.zeros(total_item_count + 1, dtype=np.int32),
    ...
    deduped = df_merged[["movie_id", "genres", "isAdult", "startYear", "startYear_raw", "genre_tokens_encoded"]].drop_duplicates(subset=["movie_id"])
    max_genres = max(
        (
            len(list(tokens)) if tokens is not None else 1
            for tokens in deduped["genre_tokens_encoded"].tolist()
        ),
        default=1,
    )
    item_features["genres_multi"] = np.zeros((total_item_count + 1, max_genres), dtype=np.int32)
    for row in deduped.itertuples(index=False):
        movie_id = int(row.movie_id)
        item_features["genres"][movie_id] = int(row.genres)
        ...
        genre_tokens = np.asarray(list(row.genre_tokens_encoded) if row.genre_tokens_encoded is not None else [int(row.genres)], dtype=np.int32)
        item_features["genres_multi"][movie_id, : genre_tokens.size] = genre_tokens
...
def build_prefix_train_eval_samples(df_merged: pd.DataFrame, total_item_count: int, max_seq_len: int, negative_rating_max: float, positive_rating_min: float) -> dict:
    ...
    for user_id_original, user_rows in df_sorted.groupby("user_id_original", sort=False):
        ...
        genre_sequence = [int(genre_id) for genre_id in user_rows["genres"].tolist()]
        rating_sequence = [float(rating) for rating in user_rows["rating"].tolist()]
        ...
        _write_sample_row(
            ...
            context_genres=genre_sequence[:test_target_idx],
        ...
        for target_idx in positive_target_indices[:-1]:
            ...
            _write_sample_row(
                ...
                context_genres=genre_sequence[:target_idx],
...
def build_item_catalog(df_merged: pd.DataFrame, total_item_count: int) -> dict:
    item_features = _build_item_feature_arrays(df_merged, total_item_count=total_item_count)
    return {
        "protocol": "item_catalog_v2",
        "all_item_ids": np.arange(1, total_item_count + 1, dtype=np.int32),
...
def run_ranking_preprocessing():
    ...
    item_catalog = build_item_catalog(df_merged, total_item_count=total_item_count)
