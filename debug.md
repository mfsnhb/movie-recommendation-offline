...
- 排序模型主要使用DIN这个序列模型，输入所有交互历史的embedding（包括交互的movie的静态特征，用户打的评分以及相对时间gap），用户的静态特征embedding以及候选movie的embedding；loss是sampled softmax而不是BCE，因为最终的目标是优化召回回来的movie的排序，而不是CTR。正样本选择大于等于4分的样本
